...
const normalizeOrderRecord = (id: string, data: Record<string, unknown>): OrderRecord => {
  ...
  return {
    ...
    organizationCheckoutRequestId: String(data.organizationCheckoutRequestId || "").trim(),
    organizationName: String(data.organizationName || "").trim(),
    exemptionCertificateNumber: String(data.exemptionCertificateNumber || "").trim(),
    taxExemptCustomer: Boolean(data.taxExemptCustomer),
