...
export const inventoryEditCard = tag(
  (
    ...
    const editFilamentHref = () => {
      if (!item?.filament_type_id) return toAdminPath("filament/types.html");
      return `${toAdminPath("filament/types.html")}?edit=${encodeURIComponent(item.filament_type_id)}`;
    };
        ...
        label(
          div.class`field-label-row`(
            ...
            a
              ...
              .href(toAdminPath("filament/types.html"))
          ...
          div.class`inventory-storage-list`(
            _=> (Array.isArray(item.storage_locations) ? item.storage_locations : []).map((entry, entryIndex) =>
              div.class`inventory-storage-row`(
                input
                  .placeholder`e.g. Shelf A / Rack 2 / Bin 3`
                  .value(() => entry?.name || "")
                  .onInput((event) =>
