import{t as h,j as b,d as t,f as y,p as s,s as v,w as _,x as w,y as u,z as o,A as k,S,e as q,B as l,c as r,g as A,h as z,v as I,O as T}from"./version-Siwik5vr.js";import{A as C}from"./AdminNav.tag-Dfgtgm47.js";import{s as P,t as R,r as U}from"./adminAppShell-Cjgh6NQy.js";import{M as $}from"./Modal.tag-D_0TQU5K.js";import"./adminNavItems-8MxaWNXe.js";import"./firebase-DNgnqO51.js";const p=[{id:"internal-organization-checkout-request",name:"Internal Organizational Checkout Request",audience:"internal",trigger:"Sent to service when an organization submits a tax-exempt checkout request.",sourceFunction:"buildOrganizationCheckoutRequestEmail",sampleInput:{requestId:"orgreq_sample_123",organizationName:"Sample Elementary PTA",organizationType:"PTA",contactName:"Sample Treasurer",contactEmail:"treasurer@example.org",phone:"555-0100",exemptionCertificateNumber:"85-8012345678C-0",certificateExpirationDate:"2027-06-30",intendedUse:"Purchase printed classroom organizers and event supplies using PTA funds.",adminReviewUrl:"https://3dlocalprint.com/admin/organization-checkout/index.html?requestId=orgreq_sample_123",createdAt:"2026-07-23T12:00:00.000Z"},variables:["adminReviewUrl","certificateExpirationDate","contactEmail","contactName","createdAt","exemptionCertificateNumber","intendedUse","organizationName","organizationType","phone","requestId"],rendered:{subject:"Organizational checkout request orgreq_sample_123 - Sample Elementary PTA",text:`Request: orgreq_sample_123
...
Review request: https://3dlocalprint.com/admin/organization-checkout/index.html?requestId=orgreq_sample_123`,html:`
...
Stripe: https://dashboard.stripe.com/test/payments/pi_sample`,html:`
  ...
  `}}];let c=document.getElementById("alertTemplatesApp");const a={current:c},i=new S(0,e=>{e.next(0)});let m=!1,x=null,f=()=>Promise.resolve(),g="";const D=e=>JSON.stringify(e,null,2),L=()=>p.find(e=>e.id===g)||null,E=e=>{g=e,i.next((Number(i.value)||0)+1)},O=()=>{g="",i.next((Number(i.value)||0)+1)},d=(e,n="")=>t.class`alert-template-source-block`(I(e),T.class`alert-template-code`(n||"No source output.")),N=e=>t.class`alert-template-detail`(t.class`alert-template-detail-header`(t(z.class`orders-detail-section-title`(e.name),s.class`orders-meta`(`${e.sourceFunction} · ${e.audience}`)),r.class(n=>`pill alert-template-audience alert-template-audience-${e.audience}`)(e.audience)),s.class`alert-template-trigger`(e.trigger),t.class`alert-template-variable-list`(e.variables.map(n=>r.class`product-category-pill`(n).key(`${e.id}-${n}`))),d("Subject",e.rendered.subject),d("Text Body",e.rendered.text),d("HTML Source",e.rendered.html),d("Sample Input",D(e.sampleInput))),M=()=>v(i,()=>{const e=L();return $({modalOpen:!!e,title:e?e.name:"Alert Template",className:"ledger-modal alert-template-modal",cardClassName:"ledger-modal-card alert-template-modal-card",bodyClassName:"alert-template-modal-body",onClose:O,content:()=>e?N(e):null})}),j=e=>u.class`ledger-row`(l(r.class`ledger-title-cell`(e.name)),l(r.class(n=>`pill alert-template-audience alert-template-audience-${e.audience}`)(e.audience)),l(r.class`orders-meta`(e.sourceFunction)),l(t.class`ledger-actions`(A.type`button`.class`ghost-button alert-template-view-button`.onClick(()=>E(e.id))("View")))).key(`row-${e.id}`),Q=()=>t.class`ledger-table-block`(t.class`ledger-table-wrap`(_.class`ledger-table alert-templates-table`(w(u(o("Template"),o("Audience"),o("Source"),o("Actions"))),k(p.map(e=>j(e))))),s.class`ledger-table-hint`(`showing ${p.length} static alert templates`)),B=h(()=>[C(f,x),b.class`panel ledger-panel alert-templates-panel`(t.class`ledger-header`(t.class`ledger-heading`(y("Alert Templates"),s("Static catalog generated from local notification code. Templates are not editable from admin."))),Q(),M())]),H=()=>{if(!a.current||m)return;const e=U(a);e&&(e.replaceChildren(),q(B,e),m=!0,c=a.current)},F=P({rootRef:a,toast:R,setAppMounted:e=>{m=e},setCurrentUser:e=>{x=e},onAfterSsoMount:()=>{c=a.current},onAuthorized:()=>{H()}});f=F.handleSignOut;
